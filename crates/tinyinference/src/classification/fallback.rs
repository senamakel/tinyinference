...
//!
//! A host can deliberately route background
...
//!
//! Hosts can enforce a local-only privacy mode at their inference chokepoint;
//! this module deliberately does not duplicate that gate.
...
/// route is unset.
pub fn role_falls_back_to_cloud(role: &str) -> bool {
...
/// name itself.
pub fn role_capability_label(role: &str) -> Option<&'static str> {
    match role.trim() {
...
pub fn override_knob_for_role(role: &str) -> &str {
    match role.trim() {
        "burst" => "agentic",
...
pub fn missing_provider_credentials_message(
    product_name: &str,
    role: &str,
    slug: &str,
    ...
    match local_chat_provider {
        Some(local) if !local.trim().is_empty() => format!(
            "No usable credentials for '{slug}', which {product_name} selected for the {} workload. \
             ...
             or enable the managed {product_name} backend.",
            role_phrase(role),
            local.trim(),
            override_knob_for_role(role),
        ),
        _ => format!(
            "No usable credentials for '{slug}', which {product_name} selected for the {} workload. \
             ...
             configured, or enable the managed {product_name} backend.",
            role_phrase(role),
            override_knob_for_role(role),
        ),
    }
...
pub fn local_vision_unsupported_message(model: &str) -> String {
    format!(
