...
const MANAGED_PROVIDER_LABEL: &str = "OpenHuman";
...
pub enum BackendErrorCode {
    /// Request throttled by the backend.
    RateLimited,
    /// User account lacks sufficient credits.
    UserInsufficientCredits,
    /// Upstream provider is temporarily unavailable.
    UpstreamUnavailable,
    /// Requested model is unavailable.
    ModelUnavailable,
    /// Request payload exceeds the backend limit.
    PayloadTooLarge,
    /// Prompt exceeds the model context window.
    ContextLengthExceeded,
    /// Request is invalid.
    BadRequest,
    /// Backend encountered an internal failure.
    InternalError,
}
...
pub fn is_managed_backend_envelope(err: &str) -> bool {
    let label = MANAGED_PROVIDER_LABEL.to_ascii_lowercase();
    let lower = err.to_ascii_lowercase();
