use crate::lm_studio::{
    LmStudioModelsResponse, apply_lm_studio_auth, lm_studio_base_url, ollama_tags_fallback_url,
};
use crate::ollama::{OllamaModelTag, OllamaTagsResponse};
    ...
    pub(in crate::service) async fn list_lm_studio_models(
        ...
        if !status.is_success() {
            ...
            if status == reqwest::StatusCode::NOT_FOUND
                && let Some(models) = self.list_ollama_tags_fallback(config, &base).await
            {
                return Ok(models);
            }
