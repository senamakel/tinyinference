...
fn extract_ollama_image_payload(image_ref: &str) -> Option<String> {
    ...
    let payload = if data_uri {
        image_ref.split_once(',')?.1.trim()
    } else {
        image_ref.trim()
    };
    if payload.is_empty() || (!data_uri && std::path::Path::new(payload).is_absolute()) {
        return None;
    }
    let valid = if payload.len() % 4 == 0 {
        STANDARD.decode(payload).is_ok()
    } else {
        STANDARD_NO_PAD.decode(payload).is_ok()
    };
    valid.then(|| payload.to_string())
}
...
impl LocalAiService {
    pub async fn vision_prompt(
        ...
        let images: Vec<String> = image_refs
            .iter()
            .filter_map(|reference| extract_ollama_image_payload(reference))
            .collect();
        if images.is_empty() {
        ...
        let payload: crate::ollama::OllamaGenerateResponse = response
            .json()
            .await
