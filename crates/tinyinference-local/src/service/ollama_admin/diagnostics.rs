...
use crate::model_requirements::{ContextEligibility, evaluate_context};
...
impl LocalAiService {
    ...
    pub(in crate::service) async fn list_models_at_with_timeout(
        &self,
        base: &str,
    ...
    pub(in crate::service) async fn fetch_model_show_at(
        &self,
        base_url: &str,
        model: &str,
    ) -> OllamaModelShow {
    ...
    async fn lm_studio_diagnostics(&self, config: &Config) -> Result<serde_json::Value, String> {
        ...
        if healthy && let Some(ref err) = models_error {
