...
use crate::install::{find_system_ollama_binary, run_ollama_install_script};
use crate::process::apply_no_window;
use crate::service::RuntimeConfig as Config;
use crate::service::paths::find_workspace_ollama_binary;

use super::super::LocalAiService;

impl LocalAiService {
    pub(in crate::service) async fn resolve_or_install_ollama_binary(
        &self,
        config: &Config,
    ...
    pub(in crate::service) async fn command_works(&self, command: &Path) -> bool {
    ...
    pub(in crate::service) async fn download_and_install_ollama(
        &self,
        config: &Config,
    ) -> Result<(), String> {
        let install_dir = crate::service::paths::workspace_ollama_dir(config);
        ...
        if crate::install::is_ollama_installer_running() {
            log::info!(
            ...
            while crate::install::is_ollama_installer_running() {
