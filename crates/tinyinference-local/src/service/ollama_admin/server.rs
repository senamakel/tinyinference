...
use crate::service::paths::ollama_spawn_marker_path;
...
impl LocalAiService {
    ...
    pub(in crate::service) async fn ensure_ollama_server_fresh(
        &self,
        config: &Config,
    ) -> Result<(), String> {
    ...
    pub(in crate::service) async fn reclaim_orphan_if_ours(&self, config: &Config) {
    ...
    pub(in crate::service) async fn start_and_wait_for_server(
        &self,
        config: &Config,
        ollama_cmd: &Path,
    ) -> Result<(), String> {
