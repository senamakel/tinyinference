...
//!
//! Uses the same transport pattern as TinyInference's language-model provider
//! adapters while remaining independent from `tinyinference-llm`.
...
use tinyinference_core::{MAX_RETRIES, backoff_ms_for_attempt};
...
/// the language-model provider adapters in `tinyinference-llm`.
///
/// # Example
/// ```no_run
/// use tinyinference_embeddings::OpenAiEmbeddingModel;
///
/// # fn main() -> tinyinference_embeddings::Result<()> {
