...
#[test]
fn aggregate_does_not_hide_transient_attempts() {
    let timeout = "The model `x` may not be available on your provider. Configure a fallback \
        chain via `reliability.model_fallbacks`. All providers/models failed. Attempts: \
        provider=custom model=x: timed out";
    let unavailable = "The model `x` may not be available on your provider. Configure a fallback \
        chain via `reliability.model_fallbacks`. All providers/models failed. Attempts: \
        provider=custom model=x: 503 Service Unavailable";
    assert!(!is_provider_config_rejection_message(timeout));
    assert!(!is_provider_config_rejection_message(unavailable));
}

#[test]
fn thinking_history_contract_failures_remain_reportable() {
